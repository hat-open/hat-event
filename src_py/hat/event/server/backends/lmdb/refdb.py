...
class RefDb:
    ...
    def add(self,
        ...
        self._changes[event.id.server][event.id] = event, set(refs)
    ...
    def remove(self,
        ...
        removed = server_changes.removed.get(event_id, set())
        ...
        if not added[1] and not removed:
            self._changes.pop(event_id, None)
...
def _query_changes(changes, server_id, max_results, last_event_id):
    ...
    if last_event_id:
        events = itertools.dropwhile(lambda i: i.id > events[-1].id,
                                     events)
