...
def ext_create(env: environment.Environment,
               ...
               partitions: typing.Iterable[Partition],
...
class TimeseriesDb:
    ...
    def add(self,
            event: common.Event
            ) -> typing.Iterable[common.EventRef]:
    ...
    async def _query_partition(self, partition_id, params, subscription,
        ...
        if params.order == common.Order.DESCENDING:
            events.extend(_query_partition_changes(
                changes[partition_id], params, subscription, max_results + 1))

            if len(events) <= max_results:
                events.extend(await self._env.execute(
                    _ext_query_partition_events, self._env, self._conditions,
                    partition_id, params, subscription,
                    max_results + 1 - len(events)))

        elif params.order == common.Order.ASCENDING:
            events.extend(await self._env.execute(
                _ext_query_partition_events, self._env, self._conditions,
                partition_id, params, subscription, max_results + 1))

            if len(events) <= max_results:
                events.extend(_query_partition_changes(
                    changes[partition_id], params, subscription,
                    max_results + 1 - len(events)))
...
def _ext_query_partition_events(env, conditions, partition_id, params,
    ...
    events = _filter_events(events, params.t_from, params.t_to,
                            params.source_t_from, params.source_t_to,
                            subscription, max_results)
...
def _query_partition_changes(changes, params, subscription, max_results):
    if params.order == common.Order.DESCENDING:
    ...
    yield from _filter_events(events, params.t_from, params.t_to,
                              params.source_t_from, params.source_t_to,
                              subscription, max_results)


def _filter_events(events, t_from, t_to, source_t_from, source_t_to,
                   subscription, max_results):
    counter = 0
    for event in events:
        ...
        if subscription and not subscription.matches(event.type):
            continue
