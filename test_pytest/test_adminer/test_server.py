...
async def test_listen(addr):
    srv = await hat.event.adminer.listen(addr)
...
async def test_get_log_conf(addr):
    ...
    await common.send_msg(conn, 'HatEventAdminer.MsgGetLogConfReq',
                          None,
                          last=False)

    msg, msg_type, msg_data = await common.receive_msg(conn)
    assert msg.last
    assert msg_type == 'HatEventAdminer.MsgGetLogConfRes'
...
async def test_set_log_conf(addr):
    ...
    await common.send_msg(conn, 'HatEventAdminer.MsgSetLogConfReq',
                          json.encode(conf),
                          last=False)

    msg, msg_type, msg_data = await common.receive_msg(conn)
    assert msg.last
    assert msg_type == 'HatEventAdminer.MsgSetLogConfRes'
    assert msg_data[0] == 'success'
