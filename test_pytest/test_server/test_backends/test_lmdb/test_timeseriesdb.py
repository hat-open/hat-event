...
from hat.event.server.backends.lmdb.timeseriesdb import Partition
...
async def create_timeseries_db(env,
    ...
    def ext_create():
        with env.ext_begin(write=True) as txn:
            return hat.event.server.backends.lmdb.timeseriesdb.ext_create(
                env=env,
                txn=txn,
                conditions=conditions,
...
async def test_add(events_count, db_path):
    ...
    assert result.events == list(reversed(events))
    assert result.more_follows is False
...
async def test_query_max_results(order, order_by, db_path):
    ...
    if order == common.Order.DESCENDING:
        assert result.events == [events[1], events[0]]
    elif order == common.Order.ASCENDING:
        assert result.events == [events[0], events[1]]
    ...
    if order == common.Order.DESCENDING:
        assert result.events == [events[2], events[1]]
    elif order == common.Order.ASCENDING:
        assert result.events == [events[0], events[1]]
    ...
    assert result.more_follows is False
    if order == common.Order.DESCENDING:
        assert result.events == [events[2], events[1], events[0]]
    elif order == common.Order.ASCENDING:
        assert result.events == [events[0], events[1], events[2]]
    ...
    assert result.more_follows is True
    if order == common.Order.DESCENDING:
        assert result.events == [events[3], events[2], events[1]]
    elif order == common.Order.ASCENDING:
        assert result.events == [events[0], events[1], events[2]]
